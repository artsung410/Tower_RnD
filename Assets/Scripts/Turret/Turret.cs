...
public class Turret : MonoBehaviour
{
    ...
    [Header("====== 범위 레이저 속성======")]
    public bool useLaser_Range = false;
    private Animator Laser_Range_animator;

    private void Start()
    {
        // 범위 레이저 터렛일때만 animator가져오기
        if (gameObject.tag == "LaserRangeTurret")
        {
            Laser_Range_animator = GetComponent<Animator>();
        }

        // 미사일 터렛일때만 dangerZone가져오기 
        if (gameObject.tag == "MissileTurret")
        {
    ...
    private void Update()
    {
        ...
        if (target == null)
        {
            ...
            if (gameObject.tag == "MissileTurret" && dangerZone.activeSelf == true)
            {
                dangerZone.SetActive(false);
                Debug.Log("false 들어옴");
            }
        ...
        if (gameObject.tag == "MissileTurret" && isEnemyInRange == true)
        {
